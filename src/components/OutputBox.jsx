  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center select-none animate-fade-up">
      ...
      <p className="text-sm text-slate-500 max-w-sm leading-relaxed">
        Fill in at least the <span className="font-semibold text-slate-700">Issue / ask</span> field
        and click <span className="font-semibold text-slate-700">Generate Prompt</span>.
      </p>
      ...
      <div className="flex flex-wrap justify-center gap-2 mt-6">
        {['Bug fix', 'Feature', 'Code review', 'Unit test'].map((tag) => (
          <span
            key={tag}
            className="text-xs font-medium px-3 py-1 bg-slate-100 text-slate-500 rounded-full border border-slate-200"
          >
            {tag}
          </span>
        ))}
    ...
    <div ref={boxRef} className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            className={`w-2.5 h-2.5 rounded-full ${output ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`}
          ></div>
        ...
        {output && (
          <div className="flex items-center gap-2 animate-fade-in">
            <button
              ...
              disabled={loading}
              className="btn-ghost !py-1.5 !px-3 !text-xs !bg-white"
            >
              {loading ? (
      ...
      {/* ── Output container ──────────────────────────────────── */}
      <div
        className={`relative rounded-xl overflow-hidden transition-all duration-300 ${output ? 'bg-slate-900 shadow-xl shadow-slate-900/10' : 'bg-slate-50 border border-slate-200 border-dashed'}`}
      >
        {output && (
        ...
        {loading && (
          <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex flex-col items-center justify-center animate-fade-in">
            <svg
              className="w-8 h-8 text-brand-600 animate-spin mb-3"
              fill="none"
              viewBox="0 0 24 24"
            >
              <circle
