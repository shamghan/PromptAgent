...
export default function HistoryPanel({ history, onLoad, onDelete, onClearAll, onClose, isOpen }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      {/* Slide panel */}
      <aside ref={panelRef} className="relative z-50 flex flex-col w-full max-w-sm h-full bg-white border-l border-slate-200 shadow-2xl animate-slide-in-right">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50/50">
          ...
          <div className="flex items-center gap-1">
            {history.length > 0 && (
              <button onClick={onClearAll} className="text-xs px-2.5 py-1.5 rounded-md font-medium text-red-600 hover:bg-red-50 transition-colors">
                Clear all
              </button>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/30">
          {history.length === 0 ? (
            ...
            history.map((entry) => (
              <div key={entry.id} className="group bg-white rounded-xl p-4 border border-slate-200 shadow-sm hover:shadow-md hover:border-brand-200 transition-all duration-200">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">{formatTimestamp(entry.timestamp)}</p>
                ...
                <p className="text-sm font-bold text-slate-800 truncate mb-1">{cardTitle(entry.inputs)}</p>
                <p className="text-[13px] leading-relaxed text-slate-500 mb-4 line-clamp-2">{entry.output?.slice(0, 90)}…</p>
                
                <div className="flex gap-2">
                  <button onClick={() => { onLoad(entry); onClose(); }} className="flex-1 py-1.5 rounded-lg text-xs font-semibold bg-brand-50 text-brand-700 hover:bg-brand-100 transition-colors">
                    Load Prompt
                  </button>
                  <button onClick={() => onDelete(entry.id)} className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-colors">
