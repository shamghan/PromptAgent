...
export function useLlm(systemPrompt) {
  ...
  const generate = useCallback(async (inputs, rawUserMessage = null) => {
    ...
    if (usesGeminiApi && (!geminiKey || geminiKey === 'your_gemini_api_key_here' || geminiKey === 'your_gemini2_api_key_here')) {
      setError('No Gemini API key found. Add VITE_GEMINI_API_KEY to your .env file and restart the dev server.');
      setLoading(false);
      return null;
    }
    ...
    try {
      ...
      if (usesGeminiApi) {
        ...
        if (!response.ok) {
          ...
          setError(`Gemini API error ${response.status}: ${errData?.error?.message || response.statusText}`);
          setLoading(false);
          return null;
        }
        ...
        if (!response.ok) {
          if (response.status === 401) {
            ...
            const errData = await response.json().catch(() => ({}));
            setError(`Groq API error ${response.status}: ${errData?.error?.message || response.statusText}`);
          }
          setLoading(false);
          return null;
        }
